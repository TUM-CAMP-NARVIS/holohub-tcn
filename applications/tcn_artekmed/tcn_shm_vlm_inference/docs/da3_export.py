#!/usr/bin/env python3
"""
Export a Depth-Anything-3 checkpoint to an ONNX model compatible with the DA3 Holoscan
fragment (`da3_fragment.py`) in this application.

This is a patched version of the upstream `export.py` from
https://github.com/ByteDance-Seed/Depth-Anything-3 (Apache-2.0). Drop it into the root of a
Depth-Anything-3 checkout and run it there. See `da3_onnx_export.md` for the full how-to.

Key differences from upstream (why this file exists): the graph accepts CHANNELS-LAST
`[B, H, W, 3]` input in `[0, 1]` -- matching the Holoscan FormatConverterOp output and the
DepthAnythingV2 ONNX contract -- and does the transpose-to-NCHW + ImageNet normalization
INSIDE the graph. The stock exporter produces an NCHW graph, which TensorRT silently
misreads (scrambled channels -> broken depth).

Example:
    python da3_export.py \
        --model-dir <DA3METRIC-LARGE snapshot dir or HF id> \
...
from depth_anything_3.model import da3 as da3mod
from depth_anything_3.utils.visualize import visualize_depth

PATCH_SIZE = 14


def _skip_sky(self, depth, *args, **kw):
    return depth


da3mod.DepthAnything3Net._process_mono_sky_estimation = _skip_sky


class DepthAnything3OnnxWrapper(nn.Module):
    """Forward that takes channels-last (B, H, W, 3) in [0, 1] and returns depth."""

    def __init__(self, api_model: DepthAnything3) -> None:
        super().__init__()
        self.model = api_model
        # ImageNet / DINOv2 normalization, baked into the graph as constants
        self.register_buffer("mean", torch.tensor([0.485, 0.456, 0.406]).view(1, 3, 1, 1))
        self.register_buffer("std", torch.tensor([0.229, 0.224, 0.225]).view(1, 3, 1, 1))

    def forward(self, image: torch.Tensor) -> torch.Tensor:  # type: ignore[override]
        # Input is channels-last (B, H, W, 3) with values in [0, 1] -- this matches the
        # Holoscan FormatConverterOp output and the DepthAnythingV2 ONNX contract. Transpose
        # to NCHW inside the graph, then apply ImageNet/DINOv2 normalization.
        image = image.permute(0, 3, 1, 2)  # (B, H, W, 3) -> (B, 3, H, W)
        image = (image - self.mean) / self.std   # normalize on (B,3,H,W) BEFORE unsqueeze
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export Depth Anything 3 to ONNX (NHWC).")
    parser.add_argument(
        "--model-dir",
        type=str,
...
def export_onnx(
    ...
    # NHWC dummy input (channels-last), matching the graph's expected layout.
    dummy_input = torch.zeros(batch_size, height, width, 3, device=device)
...
def _infer_size_from_input(sess_input, default_h: int, default_w: int) -> tuple[int, int]:
    ...
    # Expect [B, H, W, 3] (channels-last); use defaults if symbolic/None.
    h = shape[1] if isinstance(shape[1], int) else default_h
    w = shape[2] if isinstance(shape[2], int) else default_w
    return int(h), int(w)


def preprocess_image(image_path: Path, target_h: int, target_w: int):
    """Load and preprocess an image to (1, H, W, 3) float32 in [0, 1], channels-last.

    ImageNet normalization is baked into the ONNX graph, so the demo feeds *un-normalized*
    [0, 1] RGB in the same channels-last layout the exported graph expects.
    """
    ...
    tensor = tensor.permute(0, 2, 3, 1).contiguous()  # (1,H,W,3) channels-last
...
def run_onnx_demo(
    ...
    print(
        f"[DEMO] Depth stats: min={float(depth.min()):.4f}, "
        f"max={float(depth.max()):.4f}, mean={float(depth.mean()):.4f}"
    )

    # Focal scaling based on original intrinsics and resize (informational; the runtime
    # DA3PostprocessorOp does the same with the real camera focal length).
