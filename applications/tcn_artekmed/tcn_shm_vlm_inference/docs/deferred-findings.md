...
`python/operators/tcn_artekmed/tcn_langsam/models.py`: `engine_batch` is read from the profile **minimum**. An engine built
...
`python/operators/tcn_artekmed/tcn_langsam/models.py`: `token_class_ids` / `num_classes` / `_class_masks` are assigned before
...
`python/operators/tcn_artekmed/tcn_langsam/models.py`: the `except Exception` around `get_tensor_profile_shape` now logs the
...
Unreachable from `operators/tcn_artekmed/tcn_langsam/realtime.py`, which always passes a device.
...
- **`gdino_postprocess` is re-exported from `tcn_langsam.models`** with no runtime consumer — the
  tests import it directly from `tcn_langsam.helpers`, and the PyTorch backend uses HF's
...
- **`build_label_map` has no runtime caller** — superseded by `build_panoptic_map` (flat class
  ids vs the packed `(class << 8) | instance` map). Only the tests and a `tcn_langsam.models`
...
- **`worker_batch` is imported but unused** in `operators/tcn_artekmed/tcn_langsam/realtime.py` (the file computes
...
- **`python/tests/test_prompt_remap.py`** has a second `from tcn_langsam.helpers import ...` in the
...
- **The operator's `hw` is the last camera's shape** (`operators/tcn_artekmed/tcn_langsam/realtime.py`) and is then
