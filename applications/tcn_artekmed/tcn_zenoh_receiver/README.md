...
```bash
./holohub run tcn_zenoh_receiver
```
...
```bash
./holohub run tcn_zenoh_receiver --local
```
...
```bash
# Build the container
./holohub build-container tcn_zenoh_receiver

# Run the containerized application
./holohub run-container tcn_zenoh_receiver
```
...
./holohub build-container tcn_zenoh_receiver --base-image nvcr.io/nvidia/clara-holoscan/holoscan:v3.7.0-dgpu

# Run with specific GPU type
./holohub run-container tcn_zenoh_receiver --gpu-type dgpu
```
